...
# xarray is only needed to BUILD the soil table from ERA5, which happens offline
# on a workstation with a CDS key. At runtime the simulator reads the derived
# CSV and never opens a NetCDF file — so xarray and netCDF4 (~90 MB together)
# stay out of the serverless bundle. `from __future__ import annotations` above
# keeps the xr.Dataset annotations below valid as strings.
if TYPE_CHECKING:
    import xarray as xr
