...
The key is never stored in source. It is resolved, in order:
  ...
  1. ``OPENWEATHER_API_KEY`` environment variable — this is the one to set on
     Vercel, under Project Settings -> Environment Variables
  2. a ``.env`` file at the project root, line ``OPENWEATHER_API_KEY=...``
  3. a plain-text file ``openweather_key.txt`` at the project root

(3) mirrors the convention `data/era5_fetch.py` uses for the CDS key. Both files
are gitignored. The original project also read Streamlit secrets; this build has
no Streamlit, so that source is gone — see `_key_from_dotenv` below.

**No key is required.** Without one the app uses Open-Meteo, which needs none.
...
# The Streamlit-secrets lookup that the original project resolves third is
# deliberately absent from this build. There is no Streamlit here — the key comes
# from a Vercel environment variable — so probing `st.secrets` could only ever
# return None, while importing Streamlit to find that out prints a
# "missing ScriptRunContext" warning on every cold start. The remaining three
# sources are unchanged and are still tried in the same order.
...
def get_api_key() -> str:
    ...
    raise MissingAPIKeyError(
        ...
        f"  3. {PROJECT_ROOT / 'openweather_key.txt'}  (key on a single line)\n"
        f"On Vercel, (1) is the one to use: Project Settings -> Environment "
        f"Variables -> {OPENWEATHER_API_KEY_ENV}.\n"
